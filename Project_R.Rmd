...
All the data frames have 485 rows that represent the date of cases.\
...
see our data starts at the 21.1.20 and ends at the 12.6.21.
...
-   We can see that California had the highest number of cases in a
    single day, an explanation for that might be because California
    population is bigger then the others and also California is very
    touristic. **Tourism can increase the number of cases rapidly.**
...
# cast date col from chr type to date type
...
About the tweets quantity - We can see few peeks, one of them is way bigger then the others, in this range of time we can see a steep rise in the concern level of both Democrats and Republicans.\
...
### Step4: (look for variables with correlation)

### Pearson's R :

Select the variables we will use in the model.\
Try different combinations and calculate their Pearson's R :
```{r}
...
**Note:** It looks like (avg_tfidf*date_weight) gives us the best result (Pearson's R = 0.8753213) \
Take a brief look at the information for the chosen combination:

```{r}
cor.test(Data_For_Model$Normalized_Quantity+Data_For_Model$Log_Days_Past, Data_For_Model$Dem, method="pearson")
```
...
Using  **correlation matrix** ,we would like to find the variable that has the highest correlation in absolute value with Dev_Prop in order to understand the nature of their relationship we will use **cor, sapply**.\
